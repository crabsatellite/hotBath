...
public class TwilightForestMixinPlugin implements IMixinConfigPlugin {
    ...
    public boolean shouldApplyMixin(String targetClassName, String mixinClassName) {
        ...
        if (HotBathMixinPlugin.isCompatibilityModeEnabled()) {
            System.out.println("[HotBath] Twilight Forest mixin " + mixinClassName + " disabled due to compatibility mode");
            return false;
        }
        // Only apply mixins if Twilight Forest is loaded
        boolean apply = checkTwilightForestPresent();
        if (!apply) {
            System.out.println("[HotBath] Twilight Forest not loaded, skipping mixin: " + mixinClassName);
        }
        return apply;
    }
